...
Unit tests are required for all functionality except for the rare cases where the code requires Android framework dependencies that cannot be mocked. In these cases, the minimum required code should be extracted to the `com.jkuester.unlauncher.android` package.
