...
@AndroidEntryPoint
class AnalogClockTypeDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog = AlertDialog
        .Builder(context)
        .setTitle(R.string.choose_analog_clock_type_dialog_title)
        .setSingleChoiceItems(
            R.array.analog_clock_type_array,
            corePreferencesRepo.get().analogClockType.number,
            this::onSelection
        )
        .create()
    ...
    private fun onSelection(dialogInterface: DialogInterface, i: Int) = dialogInterface
        .dismiss()
        .also { corePreferencesRepo.updateAsync(setAnalogClockType(AnalogClockType.forNumber(i))) }
}
